...
static QScriptValue debuggerScriptValueToScriptValue(QScriptEngine *eng, const QScriptDebuggerValue &in)
{
    ...
    case QScriptDebuggerValue::ObjectValue:
		out.setProperty(QString::fromLatin1("value"), eng->toScriptValue(QString::number(in.objectId())));
		break;
    }
    return out;
}
...
static void debuggerScriptValueFromScriptValue(const QScriptValue &in, QScriptDebuggerValue &out)
{
    ...
    case QScriptDebuggerValue::NumberValue:
		...
		break;
    case QScriptDebuggerValue::ObjectValue:
		out = in.property(QString::fromLatin1("value")).toString().toLongLong();
		break;
	default:
...
static QScriptValue debuggerResponseToScriptValue(QScriptEngine* eng, const QScriptDebuggerResponse& in)
{
    ...
    QString strType = in.type();
    if (strType == "QScriptBreakpointData") {
        QScriptBreakpointData data;
        data.fromVariant(in.result().toMap());
        result = breakpointDataToScriptValue(eng, data);
    }
    else if (strType == "QScriptBreakpointMap") {
        result = eng->newArray();
        QVariantList list = in.result().toList();
        for (int i = 0; i < list.size(); i++) {
            QScriptBreakpointData data;
            data.fromVariant(list[i].toMap());
            result.setProperty(i, breakpointDataToScriptValue(eng, data));
        }
    }
    else if (strType == "QScriptScriptData") {
        QScriptScriptData data;
        data.fromVariant(in.result().toMap());
        result = scriptDataToScriptValue(eng, data);
    }
    else if (strType == "QScriptScriptMap") {
        result = eng->newArray();
        QVariantList list = in.result().toList();
        for (int i = 0; i < list.size(); i++) {
            QScriptScriptData data;
            data.fromVariant(list[i].toMap());
            result.setProperty(i, scriptDataToScriptValue(eng, data));
        }
    }
    else if (strType == "QScriptDebuggerContextInfo") {
        QScriptDebuggerContextInfo data;
        data.fromVariant(in.result().toMap());

        QScriptValue out = eng->newObject();
        out.setProperty(QString::fromLatin1("scriptId"), QScriptValue(eng, qsreal(data.scriptId())));
        out.setProperty(QString::fromLatin1("fileName"), QScriptValue(eng, data.fileName()));
        out.setProperty(QString::fromLatin1("lineNumber"), QScriptValue(eng, data.lineNumber()));
        out.setProperty(QString::fromLatin1("columnNumber"), QScriptValue(eng, data.columnNumber()));
        out.setProperty(QString::fromLatin1("functionName"), QScriptValue(eng, data.functionName()));
        result = out;
    }
    else if (strType == "QScriptDebuggerValue") {
        QScriptDebuggerValue data;
        data.fromVariant(in.result().toMap());
        result = debuggerScriptValueToScriptValue(eng, data);
    }
    else if (strType == "QScriptDebuggerValueList") {
        result = eng->newArray();
        QVariantList list = in.result().toList();
        for (int i = 0; i < list.size(); i++) {
            QScriptDebuggerValue data;
            data.fromVariant(list[i].toMap());
            result.setProperty(i, debuggerScriptValueToScriptValue(eng, data));
        }
    }
    else if (strType == "QScriptDebuggerValueProperty") {
        QScriptDebuggerValueProperty data;
        data.fromVariant(in.result().toMap());
        result = debuggerScriptValuePropertyToScriptValue(eng, data);
    }
    else if (strType == "QScriptDebuggerValuePropertyList") {
        result = eng->newArray();
        QVariantList list = in.result().toList();
        for (int i = 0; i < list.size(); i++) {
            QScriptDebuggerValueProperty data;
            data.fromVariant(list[i].toMap());
            result.setProperty(i, debuggerScriptValuePropertyToScriptValue(eng, data));
        }
    }
    else if (strType == "QScriptScriptsDelta") {
        QVariantMap in_result = in.result().toMap();
        QScriptScriptsDelta delta;
        foreach(const QVariant & v, in_result["added"].toList())
            delta.first.append(v.toLongLong());
        foreach(const QVariant & v, in_result["removed"].toList())
            delta.second.append(v.toLongLong());
        result = eng->toScriptValue(QVariant::fromValue(delta));
    }
    else if (strType == "QScriptContextsDelta") {
        QVariantMap in_result = in.result().toMap();
        QScriptContextsDelta delta;
        foreach(const QVariant & v, in_result["added"].toList())
            delta.first.append(v.toLongLong());
        foreach(const QVariant & v, in_result["removed"].toList())
            delta.second.append(v.toLongLong());
    ...
    else if (strType == "QScriptDebuggerObjectSnapshotDelta") {
        QScriptDebuggerObjectSnapshotDelta delta;
        delta.fromVariant(in.result().toMap());
        result = eng->toScriptValue(QVariant::fromValue(delta));
    }
    else
        result = eng->toScriptValue(in.result());

    QScriptValue out = eng->newObject();
    //out.setProperty(QString::fromLatin1("result"), eng->toScriptValue(in.result()));
    out.setProperty(QString::fromLatin1("result"), result);
    out.setProperty(QString::fromLatin1("error"), QScriptValue(eng, in.error()));
    out.setProperty(QString::fromLatin1("async"), QScriptValue(eng, in.async()));
    return out;
}
