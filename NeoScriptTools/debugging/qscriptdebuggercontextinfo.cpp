...
void QScriptDebuggerContextInfo::fromVariant(const QVariantMap& in)
{
    if (!d_ptr) {
        d_ptr.reset(new QScriptDebuggerContextInfoPrivate());
        d_ptr->ref.ref();
    }
    QScriptDebuggerContextInfoPrivate* d = d_ptr.data();
    d->scriptId = in["scriptId"].toLongLong();
    d->fileName = in["fileName"].toString();
    d->lineNumber = in["lineNumber"].toInt();
    d->columnNumber = in["columnNumber"].toInt();
    d->functionName = in["functionName"].toString();
    d->functionType = (QScriptDebuggerContextInfo::FunctionType)in["functionType"].toInt(); // todo string
}