...
//> NeoScriptTools
void QScriptDebuggerObjectSnapshotDelta::fromVariant(const QVariantMap& in)
{
    removedProperties = in["removedProperties"].toStringList();
    foreach(const QVariant & var, in["changedProperties"].toList()) {
        QScriptDebuggerValueProperty data;
        data.fromVariant(var.toMap());
        changedProperties.append(data);
    }
    foreach(const QVariant & var, in["addedProperties"].toList()) {
        QScriptDebuggerValueProperty data;
        data.fromVariant(var.toMap());
        addedProperties.append(data);
    }
}

QVariant QScriptDebuggerObjectSnapshotDelta::toVariant() const
{
    QVariantMap out;
    out["removedProperties"] = removedProperties;
    QVariantList changed;
    foreach(QScriptDebuggerValueProperty prop, changedProperties)
        changed.append(prop.toVariant());
    out["changedProperties"] = changed;
    QVariantList added;
    foreach(QScriptDebuggerValueProperty prop, addedProperties)
        added.append(prop.toVariant());
    out["addedProperties"] = added;
    return out;
}
//< NeoScriptTools
