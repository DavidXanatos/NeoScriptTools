...
{
	QJsonDocument doc = QJsonDocument::fromJson(arrJson);
	QVariantMap Variant = doc.toVariant().toMap();
	...
	if(Variant["type"] == "event")
	{
		...
		// Note: V8 does not support peristent contexts, all handles are only valid for one debugger event
		...
		else if(cmdStr == "lookup")
		{
			...
			if(typeStr == "ScriptObjectSnapshotCapture")
			{
...

				{
					...
					delta.removedProperties = QStringList(removedPropertiesSet.begin(), removedPropertiesSet.end());
